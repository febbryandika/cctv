...
// inscrutable one in apps/web. TODO: mount /recordings, /health.
//
...
//
// exposeHeaders is load-bearing for WHEP and fails the same way SPEC 9 does.
// A cross-origin response only surfaces the six CORS-safelisted headers to
// script; Location and ETag are not among them, so without this the player
// reads null for the very header the proxy exists to rewrite and the handshake
// dies with nothing obviously wrong on either side.
const app = new Hono()
  .use(
    '*',
    cors({ origin: WEB_ORIGINS, credentials: true, exposeHeaders: ['Location', 'ETag'] }),
  )
